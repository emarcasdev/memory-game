$(document).ready(function () {
    ...
    $(".card").on("click", function () {
      ...
      if (coupleActual.length === 2) {
        ...
        if (coupleActual[0].value === coupleActual[1].value) {
          score += 10; // Sumar 10pt por acierto
          ...
          if (discoverCount === couples.length) {
            ...
            setTimeout(function () {
              alert(
                `VICTORIA, ${player_name} has obtenido ${score}`
              );
  ...
  function renderCards(couples) {
    ...
    for (let i = 0; i < couples.length; i++) {
      ...
      $(".tablero").append(
        `<div class="card">
                       <div class="back">
                           <img src="../img/card-classic.png" alt="Card" style="width:100%; height:100%; object.fit:cover;">
                       </div>
  ...
  async function saveGame(player_name, score) {
    console.log("Datos enviados:", player_name, score); // Muestra los datos enviados antes de la solicitud

    try {
      const response = await fetch("../php/classicSave.php", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        let resultString = 'Los 5 mejores resultados:\n';
  ...
  async function saveDB(player_name, score) {
    try {
        const response = await fetch('../database/saveClassic.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
